...
# === Парсим HTML для удаления навигации и секций товаров ===
soup = BeautifulSoup(html_content, "html.parser")

# === Удаляем все секции товаров <section> с классом "u-section-16" ===
for old_section in soup.find_all("section", class_="u-section-16"):
...
with open(csv_path, newline="", encoding="utf-8") as csvfile:
    ...
    for row in reader:
        ...
        # Форматируем stock с поддержкой Size, Price, Place
        if not stock.strip():
            stock_html = "РАСПРОДАНО"
        else:
            stock_lines = [line.strip() for line in stock.splitlines() if line.strip()]
            size_lines = [line.strip() for line in row.get("size", "").splitlines()] if row.get("size") else []
            price_lines = [line.strip() for line in price.splitlines()] if price else []
            place_lines = [line.strip() for line in place.splitlines()] if place else []

            stock_entries = []
            for i, stock_line in enumerate(stock_lines):
                size_part = size_lines[i] if i < len(size_lines) else ""
                stock_part = stock_line
                price_part = price_lines[i] if i < len(price_lines) else ""
                place_part = place_lines[i] if i < len(place_lines) else ""
                entry = f"☀️ Размер {size_part} {stock_part} шт. за {price_part} {place_part}".strip() + "."
                stock_entries.append(entry)

            stock_html = "В наличии:<br>" + "<br>".join(stock_entries)
